 ...
 */
final class Version20210308163139 extends AbstractMigration
{
    ...
    public function up(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__user AS SELECT id, username, is_active, is_blocked, password, roles, is_verified FROM user');
        $this->addSql('DROP TABLE user');
        $this->addSql('CREATE TABLE user (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, username VARCHAR(255) NOT NULL COLLATE BINARY, is_active BOOLEAN NOT NULL, is_blocked BOOLEAN NOT NULL, password VARCHAR(255) NOT NULL COLLATE BINARY, roles CLOB NOT NULL COLLATE BINARY --(DC2Type:json)
        , is_verified BOOLEAN NOT NULL, email VARCHAR(255) NOT NULL)');
        $this->addSql('INSERT INTO user (id, username, is_active, is_blocked, password, roles, is_verified) SELECT id, username, is_active, is_blocked, password, roles, is_verified FROM __temp__user');
    ...
    public function down(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE UNIQUE INDEX UNIQ_5A8A6C8D2B36786B ON post (title)');
        $this->addSql('CREATE INDEX IDX_5A8A6C8DF675F31B ON post (author_id)');
        $this->addSql('CREATE TEMPORARY TABLE __temp__user AS SELECT id, username, roles, is_active, is_blocked, password, is_verified FROM "user"');
        $this->addSql('DROP TABLE "user"');
        ...
        , is_active BOOLEAN NOT NULL, is_blocked BOOLEAN NOT NULL, password VARCHAR(255) NOT NULL, is_verified BOOLEAN DEFAULT \'0\' NOT NULL)');
        $this->addSql('INSERT INTO "user" (id, username, roles, is_active, is_blocked, password, is_verified) SELECT id, username, roles, is_active, is_blocked, password, is_verified FROM __temp__user');
        $this->addSql('DROP TABLE __temp__user');
    }
}
